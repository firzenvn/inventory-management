{{ Form::open(array('route'=>'shipments.create', 'autocomplete'=>'off', 'method'=>'post','class'=>'form-horizontal')) }}
        ...
        <div class="wrapper wrapper-content animated fadeInRight">
            <div class="ibox-content p-xl">
                ...
                <div class="table-responsive m-t">
                    <table class="table invoice-table">
                        ...
                        <tr>
                            ...
                            <th style="width: 20%;">{{Lang::get('orders.qty_to_ship')}}</th>
                            <th>Total Price</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($model->orderItem as $item)
                            @if($item->qty_to_ship < $item->qty_add)
                            <tr>
                                <td>
                                ...
                                <td class="qty_to_add">
                                    ...
                                    {{Lang::get('orders.shipment')}} <strong>{{ $item->qty_to_ship  }}</strong> </br>
                                </td>
                                <td class="qty_to_invoice">
                                    {{ Form::hidden('product_order_item[]', $item->product_id) }}
                                    {{ Form::number('qty_to_ship['.$item->product_id.']',Input::get('qty_to_ship['.$item->product_id.']',$item->qty_add),array('style'=>'width: 25%','min'=>0,'max'=>($item->qty_add - $item->qty_to_ship))) }}
                                </td>
                ...
                <div class="text-right">
                    <button class="btn btn-primary" type="submit"><i class="fa fa-dollar"></i> Make A {{ Lang::get('orders.ship') }}</button>
                </div>
            </div>
        </div>
    </div>
