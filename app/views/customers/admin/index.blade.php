{{Form::open(array('route'=>'admin.customers.list','method'=>'get'))}}
<div class="row">
	<div class="col-lg-12">
		...
		<div class="pull-right">
			<a class="btn btn-sm btn-default" href="{{URL::route('admin.customers.list')}}">Reset</a>
		...
		<div style="margin-top: 10px">
				...
				<thead>
					...
					<tr>
						<th>#</th>
						<th>{{ Lang::get('label.client_id');BaseModel::renderHtmlSort('first_name','CustomersAdminController@getIndex') }}</th>
						<th>{{ Lang::get('customer.first_name');BaseModel::renderHtmlSort('first_name','CustomersAdminController@getIndex') }}</th>
						<th>{{ Lang::get('customer.last_name');BaseModel::renderHtmlSort('last_name','CustomersAdminController@getIndex') }}</th>
						<th>{{ Lang::get('customer.email');BaseModel::renderHtmlSort('email','CustomersAdminController@getIndex') }}</th>
						<th>{{ Lang::get('customer.customer_group_id');BaseModel::renderHtmlSort('customer_group_id','CustomersAdminController@getIndex')}}</th>
						<th>{{ Lang::get('customer.phone_no');BaseModel::renderHtmlSort('phone_no','CustomersAdminController@getIndex') }}</th>
						<th>{{ Lang::get('customer.zipcode');BaseModel::renderHtmlSort('zipcode','CustomersAdminController@getIndex') }}</th>
						<th>{{ Lang::get('customer.city');BaseModel::renderHtmlSort('city','CustomersAdminController@getIndex') }}</th>
						<th>{{ Lang::get('customer.country_id');BaseModel::renderHtmlSort('country_id','CustomersAdminController@getIndex') }}</th>
						<th>{{ Lang::get('customer.province_id');BaseModel::renderHtmlSort('province_id','CustomersAdminController@getIndex') }}</th>
						<th></th>
					</tr>
					<tr role="row">
				...
				@foreach($rows as $row)
					<tr>
						...
						<td>
							<a href="{{ URL::route('admin.customers.update',array('id'=>$row->id)) }}"><i class="fa fa-pencil-square-o"></i></a> |
							<a href="{{ URL::route('admin.customers.delete',array('id'=>$row->id)) }}" onclick="return confirm('You sure?')"> <i class="fa fa-times"></i></a>
						</td>
					</tr>
				@endforeach
				</tbody>
