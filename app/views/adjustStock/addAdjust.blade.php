	...
	<div class="col-lg-12">
		...
		<div class="ibox-content">
			<div class="" style="margin-top: 10px">
				{{ Form::open(array('action'=>'adjustStock.add_product_adjust', 'autocomplete'=>'off', 'method'=>'post','class'=>'form-horizontal','files'=>true)) }}
	...
	$('.fa-hover').click(function(){
		...
		if(a){
			...
			$.ajax({
                ...
                dataType:'json',
                data: {rid:{{ Input::get('report_id') }},id:id,switch:'adjust'}
            }).done(function(data){
                 if(data.status==1)
                 {
                    $('#tr_c_'+id).empty();
	...
	$('.fa-check').click(function(){
        ...
        if(a){
            ...
            $.ajax({
                ...
                dataType:'json',
                data: {rid:{{ Input::get('report_id') }},id:id,qty:qty,switch:'adjust'}
            }).done(function(data){
                 if(data.status==1)
                 {
                    alert(data.mes);
...
function bind(page){
	...
	$('input[name^="reviews"]').each(function() {
	 ...
	 $.ajax({
        ...
        dataType:'html',
        data: {rid:{{ Input::get('report_id') }},name:name,sku:sku,price:price,status:status,id:seq_no,all:allow,page:page,switch:'adjust'},
         beforeSend: function(){
