...
class ClientUsersAdminController extends BaseAdminController {
	...
	public function getIndex(){
		...
		$this->layout->content=View::make('client_users.admin.index')->with(array('rows'=>$rows));
	}

	public function getCreate(){
		$client = App::make('client');
		if($client->user_count >= $client->package->limit_users)
			return Redirect::route('admin.client-users.list')->with('error',Lang::get('label.limited_user'));

		$this->layout->content=View::make('client_users.admin.create');
	}

	public function postCreate(){
		$client = App::make('client');
		if($client->user_count >= $client->package->limit_users)
			return Redirect::route('admin.client-users.list')->with('error',Lang::get('label.limited_users'));
		...
		return Redirect::route('admin.client-users.list')->with('success',Lang::get('client-user.mes_add_success'));
	...
	public function getUpdate(){
		...
		if(!$this->checkOwner($id))
			return Redirect::route('admin.client-users.list');
		$model=User::find($id);
		if($model->client_id!=Auth::user()->client_id)
			return Redirect::route('admin.client-users.list');
		$this->layout->content=View::make('client_users.admin.update')->with('model',$model);
	}
	...
	public function postUpdate(){
		...
		return Redirect::route('admin.client-users.list')->with('success',Lang::get('client-user.mes_update_success'));
	}
	...
	public function getDelete()
	{
		...
		if ($model->client_id != Auth::user()->client_id)
			return Redirect::route('admin.client-users.list');
		if ($model->delete()) {
			return Redirect::route('admin.client-users.list')->with('success', Lang::get('client-user.mes_delete_success'));;
		}
	}
