...
/**
 * Check if user has permission to access an action or not
 */
Route::filter('acl', function(){
	$client=App::make('client');
	if(!$client)
		return Redirect::route('users.client-register.get');

	if(Auth::user()->id!=$client->owner_user_id) {
		$routeName = Route::currentRouteName();
		list($resource, $action) = explode('.', $routeName);
		if (Authority::cannot($action, $resource)) {
			App::abort(403, 'You are not granted to access this function');
		}
	}
});
